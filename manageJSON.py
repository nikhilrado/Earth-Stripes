...
def update_common_names(file_path="../Earth Stripes Codebase/data/location_common_names.csv"):
    with open(file_path) as csv_file:
        reader = list(csv.reader(csv_file))
        for row in reader[1:]:
            json_file = '../Earth Stripes Codebase/results/json'+row[0]+'.json'
            print(row)
            f = open(json_file)
            f = f.read()
            f = json.loads(f)
            print(json.dumps(f))
            if not "custom properties" in f:
                f['custom properties'] = {"common name":row[1]} # will create "custom properties" if it doesn't exist

            template = {'custom properties':f['custom properties'].update({'common name':row[1]})}.copy()
            template.update(f) # this allows custom property attribute to be at the top

                #f = f.update(template)
                
                #f = template.update(f) # makes it so "custom properties" is at top of dict

            with open(json_file, "w") as myFile:
                myFile.write(json.dumps(template, indent=2))
