...
function es_generate_custom_meta_tags() {
	...
	function es_state_code_to_state($state_code){
	...
	function es_country_code_to_state($country_code){
	...
	function es_get_location_name(){
		$queries = array();
		parse_str($_SERVER['QUERY_STRING'], $queries);
		$countries_with_subdivistions = array(NULL,"US");
		if(! in_array($queries['country'],$countries_with_subdivistions)){
			return(es_country_code_to_state($queries['country']));
		};
		if(is_null($queries['county'])){
			return(es_state_code_to_state($queries['state'])); //. ", " . $queries['country']);
		};
		if(! is_null($queries['county'])){
			return($queries['county'] . ", " . es_state_code_to_state($queries['state']));
		};
	}
	...
	$es_canonical_url = es_get_results_canonical_URL();
	$es_location_name = es_get_location_name();
...
add_action( 'wp_head', function(){
	if( is_page('52')) {
		...
		add_action( 'wp_head', 'es_generate_custom_meta_tags');
