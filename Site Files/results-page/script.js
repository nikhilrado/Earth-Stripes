...
function breadcrumbs(){
    ...
    if (country && state){
        ...
        breadcrumbHTML += "<a href=https://dev.earthstripes.org"+generateURL(country)+">" + countryName + "</a>"+ breadcrumbSeparator;
    }
    if(country){
      breadcrumbsSchema += `    {
      "@context": "https://schema.org",
      ...
      "itemListElement": [{
        ...
        "position": 1,
        "name": "`+countryName+`",
        "item": "https://dev.earthstripes.org`+generateURL(country)+`"
      }`
    }
    if (state && county){
      breadcrumbs.push(state)
      breadcrumbHTML += "<a href=https://dev.earthstripes.org"+generateURL(country,state)+">" + state + "</a>" + breadcrumbSeparator;
    }
    if (state){
      breadcrumbsSchema += `,{
      "@type": "ListItem",
      "position": 2,
      "name": "`+breadcrumbs[1]+`",
      "item": "https://dev.earthstripes.org`+generateURL(country,state)+`"
      }`;
    }
    if (false){
        //breadcrumbs.push(county)
        breadcrumbHTML += "<a href=https://dev.earthstripes.org"+generateURL(country,state,county)+">" + county.replaceAll("+"," ") + "</a>" + breadcrumbSeparator;
    }
    if (county){
      ...
      "position": 3,
      "name": "`+breadcrumbs[2]+`",
      "item": "https://dev.earthstripes.org`+generateURL(country,state,county)+`"
      }`
    }
...
function setYaleBars() {
    ...
    for (let i = 0; i < datas.length; i++) {
        ...
        //console.log("YaleNationalAverage" + (i + 1))
        agreeNationalAverage1.setAttributeNS(null, "x", data["YaleClimateOpinionData"]["national_data"][datas[i]]);
        //agreeNationalAverage1.setAttributeNS(null, "width", data["YaleClimateOpinionData"]["national_data"][datas[i]]+1);
        //console.log(data["YaleClimateOpinionData"]["national_data"][datas[i]])
