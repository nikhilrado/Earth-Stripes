...
def cropAndResizeSenatorImages(filepath,savepath):
    ...
    left,top,right,bottom = 0,0,width,height*.7

    im1 = im.crop((left, top, right, bottom))
    im1 = im1.resize((600, int(600*.7)))
    ...
    percentSavings = ((ogSizeKB-newSizeKB)/ogSizeKB)*100
    print("Resized: %s Size: %iKB to %iKB -- %i%% savings" % (filepath,ogSizeKB,newSizeKB,percentSavings))

    #im1.show()


for photo in globals.getAllFilesInDir("results/snap-sticker"):
